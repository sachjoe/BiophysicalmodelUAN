...
NEURON {
	...
	RANGE gbar, ena, ina
	RANGE minf, hinf, mtau, htau, mtaunattxs, htaunattxs
	THREADSAFE
}
...
PROCEDURE rates(v (mV)) {
	...
	mtau 	= mtaunattxs / (alpha_m+beta_m)				: Adapted from Sheets et al.,2007 Nav1p7 Channel
	
	
	alpha_h = (0.23688)*exp(-(v+115)/46.33)
	beta_h 	= (10.8/2.5)/(1+exp((v-11.8)/-11.998))
	htau 	= htaunattxs / (alpha_h + beta_h) 			: Adapted From Baker, 2005 nattxs.mod Channel
